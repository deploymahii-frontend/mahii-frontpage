...
const ShopOwnerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      try {
        const shopResponse = await shopAPI.getMyShops();
        const category = shopResponse.data?.shops?.[0]?.category?.toLowerCase();
        navigate(getShopDashboardPath(category));
      } catch (shopError) {
        console.error('Error fetching shop category after login:', shopError);
    ...
    try {
      ...
      if (result.success) {
        ...
        try {
          ...
          console.error('Error fetching shop category after Google login:', shopError);
          navigate('/shop/dashboard');
        }
      }
    } finally {
