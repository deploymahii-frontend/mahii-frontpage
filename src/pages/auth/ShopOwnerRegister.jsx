...
import { 
  ...
  Navigation, Search, X, Upload, Image as ImageIcon
} from 'lucide-react';
...
const ShopOwnerRegister = () => {
  ...
  const validateStep1 = () => {
    ...
    if (!(formData.location.city || formData.shopCity)) newErrors['location.city'] = 'City is required';
    if (!formData.location.area) newErrors['location.area'] = 'Area/Locality is required';
    // Coordinates are optional when address, city, and area are provided.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestBody = {
        ...
        shopCity: formData.location.city || formData.shopCity,
        shopArea: formData.location.area,
        shopLat: formData.location.lat ? parseFloat(formData.location.lat) : formData.shopLat ? parseFloat(formData.shopLat) : undefined,
        shopLng: formData.location.lng ? parseFloat(formData.location.lng) : formData.shopLng ? parseFloat(formData.shopLng) : undefined,
      ...
      const response = await authAPI.shopOwnerRegister(requestBody);
      
      if (response.data.success) {
        toast.success('Your registration successfully done. Wait for approve from admin.', { duration: 5000 });
      ...
      const message = typeof rawMessage === 'string' && /server/i.test(rawMessage)
        ? 'Your request has been sent to admin. Please wait 2-3 days for approval.'
        : rawMessage || fallback;
      toast.error(message);
    } finally {
      setLoading(false);
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 1 && (
              ...
              >
                ...
                {/* SHOP IMAGE SECTION */}
                <div className="bg-gradient-to-r from-orange-50 to-pink-50 border-2 border-dashed border-[#C2185B] rounded-lg p-6">
                  <div className="flex items-center gap-2 mb-4">
                    <ImageIcon size={20} className="text-[#C2185B]" />
                    <h4 className="font-semibold text-gray-900">Shop Image</h4>
                    <span className="text-xs bg-[#C2185B]/10 text-[#C2185B] px-2 py-1 rounded-full">Optional</span>
                  </div>
                  
                  <p className="text-sm text-gray-600 mb-4">Add an attractive image of your shop to make it stand out on the Explore page. This helps customers identify your shop better.</p>
                  ...
                  {formData.shopImage ? (
                    <div className="space-y-3">
                      ...
                      <label className="inline-flex items-center gap-2 px-4 py-2 bg-[#C2185B] text-white rounded-lg cursor-pointer hover:bg-[#E5093F] transition">
                        <Upload size={16} />
                    ...
                    <label className="flex flex-col items-center justify-center py-8 bg-white rounded-lg border-2 border-gray-200 cursor-pointer hover:border-[#C2185B] hover:bg-orange-50 transition">
                      <Upload size={32} className="text-[#C2185B] mb-2" />
