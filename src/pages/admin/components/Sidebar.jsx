...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  const currentActive = activeTab || menuItems.find((item) => location.pathname.startsWith(item.path))?.id || 'overview';
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-white dark:bg-gray-900 shadow-xl z-50 flex flex-col">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-800 space-y-2">
        <Link
          to="/help"
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
        >
          <HelpCircle size={20} />
