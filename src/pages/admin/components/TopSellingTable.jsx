...
const TopSellingTable = ({ products = [] }) => {
  const displayProducts = Array.isArray(products) && products.length ? products : fallbackProducts;
  return (
        ...
        <table className="w-full">
          ...
          <tbody>
            {displayProducts.map((product) => (
              <tr key={product.id || product.name} className="border-b border-gray-100 dark:border-gray-800">
                <td className="py-3 text-sm font-medium text-gray-900 dark:text-white">{product.name}</td>
                <td className="py-3 text-sm text-gray-600 dark:text-gray-400">{product.category || 'N/A'}</td>
                <td className="py-3 text-sm text-gray-600 dark:text-gray-400">{product.sales}</td>
                <td className="py-3 text-sm font-semibold text-gray-900 dark:text-white">{typeof product.revenue === 'number' ? `₹${product.revenue.toLocaleString()}` : product.revenue}</td>
                <td className={`py-3 text-sm ${String(product.growth).includes('+') ? 'text-green-500' : 'text-red-500'}`}>
                  {product.growth || '-'}
                </td>
                <td className="py-3">
