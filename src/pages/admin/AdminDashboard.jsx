...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const revenueData = Array.isArray(revenueResponse.data.data) ? revenueResponse.data.data : [];
      ...
      if (Array.isArray(stats.recentOrders)) {
        activities.push(...stats.recentOrders.slice(0, 3).map((order) => ({
          id: `order-${order._id}`,
          type: 'order',
      ...
      if (Array.isArray(stats.pendingShopsDetails)) {
        activities.push(...stats.pendingShopsDetails.slice(0, 2).map((shop) => ({
          id: `shop-${shop._id}`,
          type: 'shop',
          user: shop.ownerId?.name || 'Shop Owner',
          action: 'submitted a shop registration',
          amount: '',
      ...
      setTopSellingProducts(Array.isArray(stats.topProducts) ? stats.topProducts : []);
    } catch (error) {
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <Link to="/admin/settings" className="flex items-center justify-center gap-2 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition">
