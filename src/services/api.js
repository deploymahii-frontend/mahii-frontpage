    ...
    if (error.response?.status === 401) {
      ...
      if (shouldRedirect) {
        if (currentPath.startsWith('/shop')) {
          window.location.href = '/login/shopowner';
      ...
      if (currentPath !== '/login/customer' && currentPath !== '/login/shopowner' && currentPath !== '/secure-admin-portal') {
...
export const shopAPI = {
  ...
  uploadLogo: (formData) => api.post('/shops/media/logo', formData),
  uploadCover: (formData) => api.post('/shops/media/cover', formData),
  uploadExplore: (formData) => api.post('/shops/media/explore', formData),
  uploadGallery: (formData) => api.post('/shops/media/gallery', formData),
  uploadVideo: (formData) => api.post('/shops/media/video', formData),
  deleteImage: (imageId) => api.delete(`/shops/media/image/${imageId}`),
  uploadLocal: (shopId, formData) => api.post(`/shops/${shopId}/local-upload`, formData),
  deleteGalleryImage: (shopId, url) => api.delete(`/shops/${shopId}/gallery`, { data: { url } }),
  
  // Shop Edit Approval Workflow
  submitEditForApproval: (shopId, data) => api.post(`/shops/${shopId}/submit-edit`, data),
  requestOwnership: (shopId, data) => api.post(`/shops/${shopId}/request-ownership`, data),
...
export const mediaAPI = {
  ...
  uploadExplore: shopAPI.uploadExplore,
...
export const subscriptionAPI = {
  ...
  getPendingSubscriptions: (shopId) => api.get(`/subscriptions/pending/${shopId}`),
  approveSubscription: (subscriptionId) => api.post(`/subscriptions/${subscriptionId}/approve`, {}),
  rejectSubscription: (subscriptionId, reason) => api.post(`/subscriptions/${subscriptionId}/reject`, { reason }),
  getMyPendingSubscriptions: () => api.get('/subscriptions/my/pending'),
};
...
export const attendanceAPI = {
  ...
  getAnalytics: (shopId) => api.get('/attendance/analytics', { params: { shopId } }),
  ...
  updateAttendance: (data) => api.put('/attendance/update', data),
  generateQRCode: (subscriptionId) => api.get(`/attendance/qr/${subscriptionId}`),
...
export const adminAPI = {
  ...
  approveOwnershipRequest: (shopId, data) => api.post(`/shops/admin/${shopId}/approve-ownership`, data),
  rejectOwnershipRequest: (shopId, data) => api.post(`/shops/admin/${shopId}/reject-ownership`, data),
