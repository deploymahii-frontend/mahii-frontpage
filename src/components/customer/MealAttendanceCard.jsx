...
import { Coffee, Sun, Moon, CheckCircle, Clock, QrCode, AlertCircle, Loader } from 'lucide-react';
...
const MealAttendanceCard = ({ subscriptionId }) => {
  ...
  const updateAttendance = async (mealType, status) => {
    setLoading(prev => ({ ...prev, [`edit-${mealType}`]: true }));
    try {
      ...
      if (response.data.success) {
        setAttendance(prev => ({
          ...prev,
          [mealType]: { ...prev[mealType], status, markedAt: new Date() },
        }));

        const previousStatus = attendance[mealType]?.status;
        ...
        setAttendanceHistory(prev => {
          return prev.map(record => {
            if (record.mealType === mealType && new Date(record.date).toDateString() === new Date().toDateString()) {
              return { ...record, status, markedAt: new Date() };
            }
            return record;
          });
        });

        setEditMode(prev => ({ ...prev, [mealType]: false }));
        toast.success(`${mealType.charAt(0).toUpperCase() + mealType.slice(1)} attendance updated to ${status}`);
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to update attendance');
    } finally {
      setLoading(prev => ({ ...prev, [`edit-${mealType}`]: false }));
    }
  };
  ...
  const toggleEditMode = (mealType) => {
    setEditMode(prev => ({ ...prev, [mealType]: !prev[mealType] }));
  };
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
      ...
      {error && !initialLoading && (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Unable to Load Attendance</h3>
          <p className="text-gray-600 dark:text-gray-400 text-center max-w-sm mb-6">{error}</p>
          <button
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="text-center p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="text-sm text-gray-500">Breakfast History</div>
              <div className="text-lg font-semibold text-orange-600">{attendanceHistory.filter(r => r.mealType === 'breakfast').length}</div>
            </div>
            <div className="text-center p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="text-sm text-gray-500">Lunch History</div>
              <div className="text-lg font-semibold text-green-600">{attendanceHistory.filter(r => r.mealType === 'lunch').length}</div>
            </div>
            <div className="text-center p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="text-sm text-gray-500">Dinner History</div>
              <div className="text-lg font-semibold text-blue-600">{attendanceHistory.filter(r => r.mealType === 'dinner').length}</div>
            </div>
          </div>
                    ...
                    <div className="flex flex-col gap-3 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(attendance[meal.key].status)}`}>
                          {getStatusLabel(attendance[meal.key].status)}
                        </span>
                      </div>
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => toggleEditMode(meal.key)}
                          className="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600"
                        >
                          {editMode[meal.key] ? 'Cancel' : 'Edit'}
                        </button>
                        {editMode[meal.key] && (
                          <div className="grid grid-cols-2 gap-2 flex-1">
                            {['present', 'absent'].map((statusOption) => (
                              <button
                                key={statusOption}
                                onClick={() => updateAttendance(meal.key, statusOption)}
                                disabled={loading[`edit-${meal.key}`] || attendance[meal.key].status === statusOption}
                                className={`w-full px-3 py-2 rounded-lg text-sm font-medium transition ${attendance[meal.key].status === statusOption ? 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 cursor-default' : 'bg-blue-500 text-white hover:bg-blue-600'}`}
                              >
                                {statusOption === 'present' ? 'Present' : 'Absent'}
                              </button>
                            ))}
                          </div>
                        )}
      ...
      <div className="bg-white rounded-2xl p-4 mt-6 shadow-sm">
        <h4 className="text-lg font-semibold mb-4">Recent Meal History</h4>
        {attendanceHistory.length === 0 ? (
          <p className="text-sm text-gray-500">No attendance history yet.</p>
        ) : (
          <div className="space-y-3">
            {attendanceHistory.slice(0, 8).map((record, index) => (
              <div key={`${record.mealType}-${record.date}-${index}`} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700">
                <div>
                  <p className="font-medium capitalize">{record.mealType}</p>
